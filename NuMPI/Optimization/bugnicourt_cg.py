"""For implementing a generic form of Constrained Conjugate Gradient for mainly use in
calculating the optimized displacement when working with contact mechanics problem with adhesion.
This implementation is mainly based upon Bugnicourt et al. - 2018, OUT_LIN algorithm.
"""
...
def constrained_conjugate_gradients(fun, hessp,
    ...
    if mean_val is not None:
        ...
        residual[mask_nonzero] = residual[mask_nonzero] - np.mean(residual[mask_nonzero])
    ...
    for i in range(1, maxiter+1):
        ...
        if len(sig.parameters) == 2:
            ...
            raise ValueError('hessp function has to take max 1 arg (descent dir)\
        ...
        if mean_val is not None:
            # assert abs(np.mean(des_dir)) < 1e-14 * np.max(abs(des_dir))
        ...
        if mean_val is not None:
            #
            mask_nonzero = x > 0
            # TODO: means actually need knowledge on the domain size.
        ...
        '''Computing beta for updating descent direction
            ...
            denom = alpha * descent_dir_transpose . (A_inverse - d2_ψadh) . descent_dir '''
        ...
        des_dir[np.logical_not(mask_bounded)] = -residual[np.logical_not(mask_bounded)] + beta*des_dir_old[np.logical_not(mask_bounded)]
        des_dir[mask_bounded] = 0

        if callback:
            callback(x)

        # print("value of residual :: {}".format(np.max(abs(residual))))
        # print(n_iterations)
        mask = x == 0
